...
const DEFAULT_SETTINGS={sensitivity:.78,fov:78,renderScale:.9,brightness:1.08,volume:.32,voiceVolume:.85,fpsCap:120,pushToTalk:false};
...
const flashlight=new THREE.SpotLight(0xfff3bd,9.5,24,Math.PI/7,.52,1.45);
...
const flashlightFill=new THREE.PointLight(0xe7dca7,.75,3);flashlightFill.position.set(.1,-.08,-.35);camera.add(flashlightFill);
...
net.onEvent=(msg)=>{
  ...
  if(msg.type==="event"){
    ...
    if(msg.kind==="start")audio.ui(true);
    renderLobby();
    return
  }
...
function createAvatar(p){
  ...
  g.userData.visual={uniform,dark,skin,anomaly,torso,head,arms,legs,tag};g.position.set(p.x||0,0,p.z||0);scene.add(g);return g
}
...
function setRevealed(a,on){const v=a.userData.visual;if(!v)return;v.torso.material=on?v.anomaly:v.uniform;v.head.material=on?v.anomaly:v.skin;v.arms.forEach(x=>x.material=on?v.anomaly:v.uniform);v.tag.material.opacity=on?.2:.72}
...
function renderLobby(){
  ...
  $("#readyBtn").textContent=mine?.ready?"UNREADY":"READY";
  $("#readyBtn").classList.toggle("primary",!mine?.ready);
  $("#startBtn").style.display=isHost()?"block":"none";
  const humanPlayers=sorted;
  const allReady=humanPlayers.length>=2&&humanPlayers.every(p=>p.ready);
...
function showLobby(){
  controls.unlock();menu.style.display="none";lobby.hidden=false;briefing.hidden=true;pauseMenu.hidden=true;settingsPanel.hidden=true;endScreen.hidden=true;hud.hidden=true;
  clearRemotes();renderLobby()
}
...
function showEnd(winner,reason){
  ...
  $("#endLobbyBtn").style.display=isHost()?"block":"none";$("#endWait").textContent=isHost()?"Return everyone to the lobby when ready.":"Waiting for lobby leader to return the team.";
  audio.alert()
}
...
window.addEventListener("keydown",e=>{
  ...
  if(e.code==="KeyE"&&currentInteraction)net.interact(currentInteraction.kind)
});
...
controls.addEventListener("unlock",()=>{
  if(connected&&isGameplay()&&briefing.hidden&&endScreen.hidden&&settingsPanel.hidden){
    pauseMenu.hidden=false;hud.hidden=true
  }
});
...
async function join(room){
  ...
  try{
    await net.connect(base,room,name);connected=true;roomLabel.textContent=room;statusEl.textContent="Incident link established.";menu.style.display="none";
    if(net.round?.phase==="lobby")showLobby()
...
async function toggleVoice(){
  ...
  try{
    await voice.enable();voiceEnabled=true;$("#voiceBtn").textContent="DISABLE PROXIMITY VOICE";$("#voiceState").textContent=settings.pushToTalk?"Voice enabled. Hold V to talk.":"Voice enabled. Nearby players can hear you.";audio.ui(true)
...
for(const [id,key,parse] of [
  ...
  ["brightnessSetting","brightness",Number],["volumeSetting","volume",Number],["voiceVolumeSetting","voiceVolume",Number],
  ["fpsSetting","fpsCap",Number]
]){
...
function resize(){
  ...
  renderer.setPixelRatio(Math.min(devicePixelRatio*Math.max(.5,Number(settings.renderScale)||.9),1.55));
...
function updateMovement(dt){
  ...
  const speed=crouch?1.8:sprint?5.8:3.45;
  if(sprint&&moving)stamina=Math.max(0,stamina-dt*20);else stamina=Math.min(100,stamina+dt*(moving?8:13));staminaEl.textContent=Math.round(stamina);
  ...
  const old=camera.position.clone(),desired=old.clone().addScaledVector(move,speed*dt),resolved=resolveMovement(old,desired,world.colliders,.31);
  camera.position.x=resolved.x;camera.position.z=resolved.z;
  if(moving){walkPhase+=dt*(sprint?12.5:crouch?5:8);audio.step(sprint)}
  const baseY=crouch?1.15:1.66,bob=moving?Math.sin(walkPhase)*(.016*(sprint?1.8:1)):0;camera.position.y=THREE.MathUtils.lerp(camera.position.y,baseY+bob,.2);
...
function updateRemoteAnimations(dt,now){
  for(const a of remotes.values()){
    const speed=a.position.distanceTo(a.userData.target)/Math.max(dt,.001);a.position.lerp(a.userData.target,Math.min(1,dt*10));
    let dy=((a.userData.yaw||0)-a.rotation.y+Math.PI)%(Math.PI*2)-Math.PI;a.rotation.y+=dy*Math.min(1,dt*9);
    const v=a.userData.visual;if(v){const phase=now*.008+a.position.x+a.position.z,amp=Math.min(.5,speed*.034);v.arms[0].rotation.x=Math.sin(phase)*amp;v.arms[1].rotation.x=-Math.sin(phase)*amp;v.legs[0].rotation.x=-Math.sin(phase)*amp;v.legs[1].rotation.x=Math.sin(phase)*amp;a.position.y=THREE.MathUtils.lerp(a.position.y,a.userData.dead?-.72:0,.09);a.visible=!a.userData.escaped}
  }
}
...
function tick(now){
  ...
  if(connected&&controls.isLocked&&self&&!self.dead&&!self.escaped&&isGameplay()&&now-lastSend>110){
