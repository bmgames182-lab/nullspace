...
const POLYHAVEN={
  carpetDiffuse:"https://dl.polyhaven.org/file/ph-assets/Textures/jpg/1k/dirty_carpet/dirty_carpet_diff_1k.jpg",
  carpetNormal:"https://dl.polyhaven.org/file/ph-assets/Textures/jpg/1k/dirty_carpet/dirty_carpet_nor_gl_1k.jpg",
  wallpaperNormal:"https://dl.polyhaven.org/file/ph-assets/Textures/jpg/1k/decrepit_wallpaper/decrepit_wallpaper_nor_gl_1k.jpg"
};
const loader=new GLTFLoader(),textureLoader=new THREE.TextureLoader(),cache=new Map();
...
async function loadAsset(key){
  ...
  const promise=loader.loadAsync(BASE+def.file).then(gltf=>{const root=gltf.scene;root.traverse(o=>{if(!o.isMesh)return;o.castShadow=false;o.receiveShadow=false;o.frustumCulled=true;if(o.material){o.material=o.material.clone();if("roughness" in o.material)o.material.roughness=Math.max(.62,o.material.roughness??.8)}});return root});cache.set(key,promise);return promise
}
function loadTexture(url,srgb=false,repeatX=1,repeatY=1){
  return new Promise((resolve,reject)=>textureLoader.load(url,t=>{t.wrapS=t.wrapT=THREE.RepeatWrapping;t.repeat.set(repeatX,repeatY);t.anisotropy=4;if(srgb)t.colorSpace=THREE.SRGBColorSpace;resolve(t)},undefined,reject))
}
async function applyRealSurfaceDetail(scene){
  try{
    const [carpetDiffuse,carpetNormal,wallNormal]=await Promise.all([
      loadTexture(POLYHAVEN.carpetDiffuse,true,16,16),loadTexture(POLYHAVEN.carpetNormal,false,16,16),loadTexture(POLYHAVEN.wallpaperNormal,false,1.4,.8)
    ]);
    const wallMaterials=new Set();let floorMaterial=null;
    scene.traverse(o=>{
      if(!o.isMesh||o.userData.nullspaceProp||o.isInstancedMesh||!o.material)return;
      const box=new THREE.Box3().setFromObject(o),size=new THREE.Vector3();box.getSize(size);
      if(size.x>30&&size.z>30&&box.max.y<.12)floorMaterial=o.material;
      else if(size.y>2.5&&Math.min(size.x,size.z)<.35&&Math.max(size.x,size.z)<5.2)wallMaterials.add(o.material)
    });
    if(floorMaterial){floorMaterial.map=carpetDiffuse;floorMaterial.normalMap=carpetNormal;floorMaterial.normalScale?.set(.38,.38);floorMaterial.color?.setHex(0xb9ad7b);floorMaterial.roughness=.98;floorMaterial.needsUpdate=true}
    for(const m of wallMaterials){if(!("normalMap" in m))continue;m.normalMap=wallNormal;m.normalScale?.set(.22,.22);m.roughness=.94;m.needsUpdate=true}
    window.__NULLSPACE_PBR__=true
  }catch(err){console.warn("NULLSPACE PBR surface maps unavailable; procedural materials retained.",err)}
}
...
function addMissingCeilingTiles(scene,cells){
  const mat=new THREE.MeshBasicMaterial({color:0x090a08,side:THREE.DoubleSide,toneMapped:false}),geom=new THREE.PlaneGeometry(1.55,1.55);const picks=[cells[1],cells[5],cells[9],cells[13]].filter(Boolean);
  for(const [i,c] of picks.entries()){const tile=new THREE.Mesh(geom,mat);tile.rotation.x=Math.PI/2;tile.position.set(c.x+(i%2?.55:-.45),3.065,c.z+(i%2?-.4:.5));tile.userData.nullspaceProp=true;scene.add(tile)}
}
function addHangingCable(scene,cells){
  const mat=new THREE.MeshBasicMaterial({color:0x111211});for(const c of [cells[4],cells[11]].filter(Boolean)){const curve=new THREE.CatmullRomCurve3([new THREE.Vector3(c.x-1.35,3.02,c.z-.3),new THREE.Vector3(c.x-.55,2.66,c.z-.08),new THREE.Vector3(c.x+.25,2.79,c.z+.1),new THREE.Vector3(c.x+1.25,3.02,c.z+.3)]);const cable=new THREE.Mesh(new THREE.TubeGeometry(curve,12,.014,5,false),mat);cable.userData.nullspaceProp=true;scene.add(cable)}
}
function addDust(scene){
  const count=520,pos=new Float32Array(count*3);for(let i=0;i<count;i++){pos[i*3]=-19+Math.random()*38;pos[i*3+1]=.16+Math.random()*2.72;pos[i*3+2]=-19+Math.random()*38}
  const geo=new THREE.BufferGeometry();geo.setAttribute("position",new THREE.BufferAttribute(pos,3));const mat=new THREE.PointsMaterial({color:0xd8cc94,size:.018,transparent:true,opacity:.18,depthWrite:false,sizeAttenuation:true});const dust=new THREE.Points(geo,mat);dust.userData.nullspaceProp=true;scene.add(dust)
}
function addPaperTrail(scene,cells){
  const geom=new THREE.PlaneGeometry(.22,.3),mat=new THREE.MeshStandardMaterial({color:0xb8ad82,roughness:1,side:THREE.DoubleSide});for(let i=0;i<12;i++){const c=cells[(i*3+2)%cells.length];if(!c)continue;const p=new THREE.Mesh(geom,mat);p.rotation.x=-Math.PI/2;p.rotation.z=(i*.91)%Math.PI;p.position.set(c.x+(Math.sin(i*2.2)*.75),.018,c.z+(Math.cos(i*1.7)*.7));p.userData.nullspaceProp=true;scene.add(p)}
}
...
export async function decorateScene(scene){
  ...
  applyRealSurfaceDetail(scene);addAtmosphere(scene,cells);
  ...
  window.__NULLSPACE_PROP_STATS__={loaded:placed.length,total:jobs.length,atmosphere:true,pbr:!!window.__NULLSPACE_PBR__};return placed
}
