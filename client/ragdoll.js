...
export class CombatRagdoll{
  constructor(scene,physics,RAPIER,{x=0,y=0,z=0,team="red",yaw=0,recoverable=false}={}){
    ...
    // The old body started with its shins intersecting the floor, which could launch corpses.
    // Spawn the whole articulated body slightly above the floor and let gravity settle it.
    const lift=.18;
    ...
    // A stiff torso/pelvis core gives the ragdoll believable mass instead of a rubber waist.
  ...
  brace(dt){
    ...
    // A weak virtual vestibular system: enough to make a wounded body resist falling and roll
    // toward upright, but intentionally too weak to look like levitation after a hard impact.
    const q=torso.rotation();tmpQ.set(q.x,q.y,q.z,q.w);tmpUp.copy(UP).applyQuaternion(tmpQ);tmpAxis.copy(tmpUp).cross(UP);const av=torso.angvel();const gain=Math.max(.25,1-this.age*.28);torso.applyTorqueImpulse({x:tmpAxis.x*.09*gain-av.x*.0025,y:-av.y*.0015,z:tmpAxis.z*.09*gain-av.z*.0025},true);
    const p=torso.translation(),pp=pelvis.translation();if(this.age<1.15&&p.y<1.17)torso.applyImpulse({x:0,y:Math.min(.045,(1.25-p.y)*.05),z:0},true);if(this.age<1.0&&pp.y<.76)pelvis.applyImpulse({x:0,y:Math.min(.038,(.84-pp.y)*.045),z:0},true)
  }
