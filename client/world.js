...
export function buildArena(scene,physics,RAPIER,{highQuality=true}={}){
  ...
  // Physics collision and 2D navigation collision are separate concerns. Horizontal surfaces such
  // as the floor need a Rapier collider for ragdolls, but must never be inserted into the X/Z
  // obstacle list used by player movement and AI line-of-sight.
  function fixedBox(x,y,z,w,h,d,mat=concrete,physicsCollide=true,navCollide=physicsCollide){
    const mesh=new THREE.Mesh(new THREE.BoxGeometry(w,h,d),mat);mesh.position.set(x,y,z);mesh.receiveShadow=highQuality;mesh.castShadow=highQuality&&h>.7;scene.add(mesh);
    if(physicsCollide){const rb=physics.createRigidBody(RAPIER.RigidBodyDesc.fixed().setTranslation(x,y,z));physics.createCollider(RAPIER.ColliderDesc.cuboid(w/2,h/2,d/2).setFriction(.9),rb)}
    if(navCollide)obstacles.push({minX:x-w/2,maxX:x+w/2,minZ:z-d/2,maxZ:z+d/2,mesh});
    shootableMeshes.push(mesh);return mesh
  }
  ...
  function lamp(x,z,color=0xffdfaa,intensity=2.1){fixedBox(x,2.75,z,.12,5.5,.12,metal,false,false);const l=new THREE.PointLight(color,intensity,13,2);l.position.set(x,4.8,z);scene.add(l)}

  // Floor: physical for ragdolls/crates, explicitly NOT an X/Z navigation obstacle.
  fixedBox(0,-.15,0,70,.3,70,groundMat,true,false);
  fixedBox(0,2,-35,70,4,.45,darkConcrete);fixedBox(0,2,35,70,4,.45,darkConcrete);fixedBox(-35,2,0,.45,4,70,darkConcrete);fixedBox(35,2,0,.45,4,70,darkConcrete);

  // West and east warehouse shells. Door openings remain physically open.
  for(const side of [-1,1]){const cx=side*13;fixedBox(cx,1.6,-12,14,3.2,.4,concrete);fixedBox(cx,1.6,12,14,3.2,.4,concrete);fixedBox(cx-side*7,1.6,0,.4,3.2,24,concrete);fixedBox(cx+side*7,1.6,-7,.4,3.2,10,concrete);fixedBox(cx+side*7,1.6,7,.4,3.2,10,concrete);fixedBox(cx,3.22,0,14,.18,24,darkConcrete,false,false);fixedBox(cx-side*2.6,1.45,-4.8,.3,2.9,7.2,concrete);fixedBox(cx+side*2.4,1.45,4.6,.3,2.9,8,concrete);fixedBox(cx-side*.8,.55,0,4.6,1.1,.55,concrete);lamp(cx-side*3,-8,0xffcfa0,1.35);lamp(cx+side*3,8,0xffcfa0,1.35)}
  ...
  for(const [x,z] of [[-14,-3],[-9,16],[14,3],[9,-16],[-4,10],[5,-9]]){fixedBox(x,.65,z,2.3,1.3,.4,tarp);fixedBox(x+.85,1.1,z+.15,.5,2.2,.5,metal,false,false)}
  ...
  for(let i=0;i<16;i++){const h=6+Math.random()*11,w=3+Math.random()*5,x=-55+Math.random()*110,z=47+Math.random()*24;fixedBox(x,h/2,z,w,h,w,darkConcrete,false,false)}
...
export function moveCircle(from,to,obstacles,r=.32){
  ...
  z=to.z;
  if(collides(x,z,obstacles,r))z=from.z;
